...
namespace ServiceFabric.Remoting.CustomHeaders
{
    ...
    internal class MethodNameProvider
    {
        ...
        {
            if (!idToMethodNameMap.TryGetValue(interfaceId, out Dictionary<int, string> methodMap)) return null;
            return methodMap.TryGetValue(methodId, out string methodName) ? methodName : null;
        }
        ...
        public void AddMethodsForProxyOrService(IEnumerable<Type> interfaces, Type baseInterfaceType)
        {
            foreach (Type interfaceType in interfaces)
            {
                ...
                if (idToMethodNameMap.TryGetValue(interfaceId, out Dictionary<int, string> methodMap)) continue;
